extern crate gl as internal_gl;
extern crate imgui;

use internal_gl::bindings::open_gl as gl;
use internal_gl::bindings::open_gl::types::*;
...
impl Renderer {
    ...
    F: FnMut(&'static str) -> *const ::std::os::raw::c_void,
  {
    ...
    unsafe {
      ...
      let vert_sources = [glsl_version.as_ptr() as *const GLchar, vert_source.as_ptr() as *const GLchar];
      let vert_sources_len = [glsl_version.len() as GLint - 1, vert_source.len() as GLint - 1];
      let frag_sources = [glsl_version.as_ptr() as *const GLchar, frag_source.as_ptr() as *const GLchar];
      let frag_sources_len = [glsl_version.len() as GLint - 1, frag_source.len() as GLint - 1];
