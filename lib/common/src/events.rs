...
use log::debug;
use shipyard::{NonSendSync, UniqueViewMut};
...
impl<T> fmt::Debug for EventId<T> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "event<{}>#{}", std::any::type_name::<T>().split("::").last().unwrap(), self.id,)
  }
}
...
impl<T> EventReader<T> {
  ...
  /// event counter, which means subsequent event reads will not include events that happened before now.
  pub fn find_latest<'a>(&mut self, events: &'a Events<T>, predicate: impl FnMut(&&T) -> bool) -> Option<&'a T> {
    self.find_latest_with_id(events, predicate).map(|(event, _)| event)
  }

  /// Like [`find_latest`](Self::find_latest), except also returning the [`EventId`] of the event.
  pub fn find_latest_with_id<'a>(&mut self, events: &'a Events<T>, mut predicate: impl FnMut(&&T) -> bool) -> Option<(&'a T, EventId<T>)> {
    self.iter_internal(events).rev().find(|(event, _id)| predicate(event)).map(|(event, id)| {
      debug!("EventReader::find_latest() -> {}", id);
      (event, id)
    })
  }
...
impl<T> Events<T> {
  ...
  pub fn update_system(mut events: NonSendSync<UniqueViewMut<Self>>) {
    events.update();
  }
...
mod tests {
  ...
  fn get_events(events: &Events<TestEvent>, reader: &mut EventReader<TestEvent>) -> Vec<TestEvent> {
