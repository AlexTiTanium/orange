...
#[cfg(feature = "winit-19")]
use winit::{
    DeviceEvent, ElementState, Event, KeyboardInput, MouseButton, MouseCursor, MouseScrollDelta, TouchPhase, VirtualKeyCode, Window, WindowEvent,
};

#[cfg(feature = "winit-20")]
use winit_20::{
    error::ExternalError,
    event::{DeviceEvent, ElementState, Event, KeyboardInput, MouseButton, MouseScrollDelta, TouchPhase, VirtualKeyCode, WindowEvent},
...
impl WinitPlatform {
    ...
    pub fn init(imgui: &mut Context) -> WinitPlatform {
        ...
        imgui.set_platform_name(Some(ImString::from(format!("imgui-winit-support {}", env!("CARGO_PKG_VERSION")))));
        WinitPlatform {
    ...
    pub fn scale_size_from_winit(&self, window: &Window, logical_size: LogicalSize) -> LogicalSize {
        match self.hidpi_mode {
            ...
            _ => logical_size.to_physical(window.get_hidpi_factor()).to_logical(self.hidpi_factor),
        }
    }
    /// Scales a logical size coming from winit using the current DPI mode.
    ///
    ...
    /// your application to use the same logical coordinates as imgui-rs.
    #[cfg(feature = "winit-19")]
    pub fn scale_pos_from_winit(&self, window: &Window, logical_pos: LogicalPosition) -> LogicalPosition {
        match self.hidpi_mode {
            ActiveHiDpiMode::Default => logical_pos,
            _ => logical_pos.to_physical(window.get_hidpi_factor()).to_logical(self.hidpi_factor),
        }
    }
    /// Scales a logical position coming from winit using the current DPI mode.
    ///
    ...
    /// your application to use the same logical coordinates as imgui-rs.
    #[cfg(feature = "winit-19")]
    pub fn scale_pos_for_winit(&self, window: &Window, logical_pos: LogicalPosition) -> LogicalPosition {
        match self.hidpi_mode {
            ActiveHiDpiMode::Default => logical_pos,
            _ => logical_pos.to_physical(self.hidpi_factor).to_logical(window.get_hidpi_factor()),
        }
    }
    /// Scales a logical position for winit using the current DPI mode.
    ///
    ...
    pub fn handle_event<T>(&mut self, io: &mut Io, window: &Window, event: &Event<T>) {
        match *event {
            ...
            // Event::DeviceEvent {
            //     event: DeviceEvent::ModifiersChanged(ModifiersState),
            //     ..
            // } => {
            //     // io.key_shift = ModifiersState.shift();
            //     // io.key_ctrl = ModifiersState.ctrl();
            //     // io.key_alt = ModifiersState.alt();
            //     // io.key_super = ModifiersState.logo();
            // }
            _ => (),
        }
    }
    ...
    pub fn prepare_frame(&self, io: &mut Io, window: &Window) -> Result<(), String> {
        if io.want_set_mouse_pos {
            let logical_pos = self.scale_pos_for_winit(window, LogicalPosition::new(f64::from(io.mouse_pos[0]), f64::from(io.mouse_pos[1])));
